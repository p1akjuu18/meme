    ...
    def batch_query_from_excel(self, file_name):
        ...
        try:
            ...
            if self.results:
                ...
                output_file = f"token_query_results_all_{timestamp}.xlsx"
                output_path = os.path.join(desktop_path, output_file)
                
                results_df = pd.DataFrame(self.results)
                results_df.to_excel(output_path, index=False)
                print(f"\n所有查询结果已保存到桌面: {output_file}")
                print(f"总记录数: {len(self.results)}")
                
                # 然后进行筛选并保存筛选后的结果
                filtered_results = [result for result in self.results 
                                 if result.get('交易池_交易量_1小时') is not None 
                                 and result.get('交易池_交易量_1小时') != 0
                                 and result.get('交易池_交易次数_1小时_卖出', 0) >= 30
                                 and isinstance(result.get('完全稀释估值(USD)', 0), (int, float))
                                 and float(result.get('完全稀释估值(USD)', 0)) > 900000]
                
                filtered_file = f"token_query_results_filtered_{timestamp}.xlsx"
                filtered_path = os.path.join(desktop_path, filtered_file)
                
                filtered_df = pd.DataFrame(filtered_results)
                filtered_df.to_excel(filtered_path, index=False)
                print(f"筛选后结果已保存到桌面: {filtered_file}")
                print(f"筛选后记录数: {len(filtered_results)}")
...
if __name__ == "__main__":
    coingecko = CoinGeckoTest(api_key="CG-dqkAyrELm8txWrs32GTwrbx4")  # 添加 API key
    coingecko.batch_query_from_excel("processed_links_20250108_2041.xlsx")